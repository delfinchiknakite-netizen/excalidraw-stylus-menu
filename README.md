...
**Тап пером по объекту** (касание без движения) → всплывающее меню действий. Меню зависит от типа:

- **Фигура**: **стрелка к объекту…** (затем тапните целевой объект — стрелка свяжет оба),
  стикер на объект, дублировать, удалить.
- **Стрелка/линия**: стикер, **наконечники** (в конце / с обеих сторон / в начале /
  точка+стрелка / без), **формат линии** (сплошная / пунктир / точки / толще / тоньше),
  дублировать, удалить.

Подпись делается стикером (текст в рамке) — обычный «голый» текст на этом устройстве Excalidraw
не сохраняется, поэтому используется стикер.
