...
- **тап кнопкой** (нажать-отпустить без движения) → меню вставки в позиции пера (у края блока — стрелка-коннектор);
- **свайп с зажатой кнопкой вправо** → redo, **влево** → undo (порог свайпа настраивается).

Касание пера к экрану — обычное рисование Excalidraw (меню по кнопке при касании не открывается,
лишь гасится родное контекстное меню). Палец не перехватывается совсем.
