...
- **двойной тап кнопкой** → переключение **перо ⇄ ластик**;
- **удержание кнопки** (~0.45 c) → **меню инструментов** у кончика пера: перо, ластик, выделение,
  **рука (двигать холст вместо рисования)**, прямоугольник, эллипс, стрелка, линия, текст.

Переключение инструментов и «вписать/двигать» идут через API Excalidraw (`setActiveTool`) — надёжно,
ничего не рисуется на холсте. Касание пера к экрану — обычное рисование Excalidraw. Палец не
перехватывается совсем.
