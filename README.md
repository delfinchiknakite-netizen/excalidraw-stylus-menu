...
По умолчанию — **боковая кнопка S Pen** (жест `penbutton`):

- **тап кнопкой при парении** (перо над холстом, без касания: нажать-отпустить) → меню вставки в позиции пера (у края блока — стрелка-коннектор);
- **касание с зажатой кнопкой + свайп вправо** → redo, **влево** → undo (порог свайпа настраивается).

Касание пера без кнопки — обычное рисование Excalidraw. Палец не перехватывается совсем.
